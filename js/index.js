...
const itemWidth =
  slideItem[0].offsetWidth +
  parseInt(
    window.getComputedStyle(slideItem[0]).getPropertyValue("margin-right")
  );
...
const noIconAcd = document.querySelectorAll(
  ".accordion-simple .accordion-item"
);
const noIconAcdBtn = document.querySelectorAll(
  ".accordion-simple .accordion-head"
);
const noIconAcdBody = document.querySelectorAll(
  ...
  if (!isPlay) {
    ...
    graph.forEach((el, index) => {
      const chartAct = setInterval(() => {
        if (i <= percent[index]) {
          el.style.background =
            " linear-gradient(to top,#3066E2 " + i + "%, #C2DAF6 " + i + "%)";
          i++;
        } else {
